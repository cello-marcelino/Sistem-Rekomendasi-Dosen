  ...
  <div class="max-w-6xl mx-auto space-y-8 animate-fade-in">
    <div class="border-b border-gray-200  pb-5">
      <h1 class="text-2xl font-bold text-gray-900  flex items-center gap-2">
        <svg class="w-7 h-7 text-amber-600 " fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"></path></svg>
    ...
    <div class="flex items-center justify-center w-full mb-8">
      <ol class="flex items-center w-full max-w-3xl">
        <li class="flex w-full items-center text-blue-600  after:content-[''] after:w-full after:h-1 after:border-b after:border-blue-100 after:border-4 after:inline-block :border-blue-800">
          <span class="flex items-center justify-center w-10 h-10 bg-blue-100 rounded-full lg:h-12 lg:w-12  shrink-0">
            1
          </span>
        </li>
        <li class="flex w-full items-center after:content-[''] after:w-full after:h-1 after:border-b after:border-4 after:inline-block" :class="step >= 2 ? 'text-blue-600  after:border-blue-100 :border-blue-800' : 'text-gray-500  after:border-gray-100 :border-gray-700'">
          <span class="flex items-center justify-center w-10 h-10 rounded-full lg:h-12 lg:w-12 shrink-0" :class="step >= 2 ? 'bg-blue-100 ' : 'bg-gray-100 '">
            2
          </span>
        </li>
        <li class="flex items-center" :class="step >= 3 ? 'text-blue-600 ' : 'text-gray-500 '">
          <span class="flex items-center justify-center w-10 h-10 rounded-full lg:h-12 lg:w-12 shrink-0" :class="step >= 3 ? 'bg-blue-100 ' : 'bg-gray-100 '">
            3
          </span>
        </li>
      </ol>
    ...
    <!-- Step 1: Upload -->
    <div v-if="step === 1" class="bg-white  rounded-xl p-8 border border-gray-100  shadow-sm max-w-2xl mx-auto">
      <div class="text-center mb-6">
        <h2 class="text-xl font-semibold text-gray-900 ">Upload Daftar Peserta Ujian</h2>
        <p class="text-gray-500  mt-1">Gunakan format template Excel yang disediakan.</p>
      </div>

      <div class="mb-6 flex justify-center">
        <button @click="downloadTemplate" class="text-sm px-4 py-2 bg-gray-50 hover:bg-gray-100 text-gray-700  :bg-gray-900  rounded-lg border border-gray-200  font-medium flex items-center gap-2 transition-colors">
      ...
      <div 
        @click="triggerUpload"
        class="border-2 border-dashed border-gray-300  rounded-xl p-10 flex flex-col items-center justify-center cursor-pointer hover:border-amber-500 hover:bg-amber-50/50 :border-amber-500 :bg-amber-900/10 transition-colors mb-6"
      >
    ...
    <!-- Step 2: Processing -->
    <div v-if="step === 2" class="bg-white  rounded-xl p-12 border border-gray-100  shadow-sm max-w-2xl mx-auto flex flex-col items-center justify-center text-center">
      ...
      <h2 class="text-2xl font-bold text-gray-900  mb-2">Memproses Penjadwalan</h2>
      <p class="text-gray-500  max-w-sm">Mencari dosen penguji yang paling relevan dengan topik TA dan mengatur slot waktu yang tidak berbenturan...</p>
    </div>

    <!-- Step 3: Result -->
    <div v-if="step === 3" class="bg-white  rounded-xl overflow-hidden border border-gray-100  shadow-sm">
      <div class="p-6 sm:px-8 border-b border-gray-100  flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-gray-50 ">
        <div>
          <h2 class="text-lg font-bold text-gray-900 ">Jadwal Ujian & Penguji Berhasil Dibuat</h2>
          <p class="text-sm text-gray-500  mt-1">Total {{ resultData.length }} jadwal ujian telah dialokasikan.</p>
        </div>
        <div class="flex gap-3">
          <button @click="step = 1" class="px-4 py-2 bg-white  border border-gray-300  text-gray-700  rounded-lg text-sm font-medium hover:bg-gray-50 :bg-gray-700 transition-colors">
            Upload Ulang
          </button>
      ...
      <div class="overflow-x-auto">
        <table class="w-full text-sm text-left text-gray-500 ">
          ...
          <tbody>
            <tr v-for="(row, idx) in resultData" :key="idx" class="bg-white  border-b  hover:bg-gray-50 :bg-gray-800/50">
              <td class="px-6 py-4">
                <div class="font-medium text-gray-900 ">{{ row.nama_mahasiswa }}</div>
              ...
              <td class="px-6 py-4">
                <div class="font-medium text-gray-900 ">{{ row.tanggal }}</div>
